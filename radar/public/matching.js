(function (root, factory) {
  ...
  else root.RadarMatching = factory();
...
const MATCH_SCHEMA_VERSION = 1; // Legacy local-cache import format.
...
function candidateBrief(profile, preferencesText) {
   ...
   * longer answers that. */
  if (profile.prose && profile.prose.trim()) {
    return `PROFILE FACTS AND CONSTRAINTS:\n${stableStringify(profile.core || {})}\n\n${profile.prose.trim()}`;
  }
...
// Canonical JSON makes object property order irrelevant. Preserve array order:
// it can express priority, such as the candidate's ordered capabilities.
...
async function fingerprint(value) {
  ...
  return `judge${JUDGMENT_VERSION}:sha256:` + Array.from(new Uint8Array(digest),
    (byte) => byte.toString(16).padStart(2, '0')).join('');
}

// Both prompts, the response schema, model and candidate input identify a
// judgment generation. Old rows stay stored and are never treated as current.
function judgmentContract(model = DEFAULT_MODEL) {
  return { version: JUDGMENT_VERSION, model, system: JUDGE_SYSTEM_PROMPT,
    schema: JUDGMENT_SCHEMA, userTemplate: judgeUserPrompt.toString() };
}
function contractFingerprint(model = DEFAULT_MODEL) {
  return fingerprint(judgmentContract(model));
}
function profileFingerprint(profile, model = DEFAULT_MODEL) {
  return fingerprint({ ...judgmentContract(model), candidate: candidateBrief(profile, ''),
    core: profile.core || {}, variants: profile.variants || [] });
}

function jobInput(job) {
  return { title: job.title || '', department: job.department || '',
    description: job.description_text || '', employer: job.employer_name || '',
    location: job.location || '', remote: Boolean(job.remote),
    salary_min: job.salary_min ?? null, salary_max: job.salary_max ?? null,
    // Include the rendered input too, so changes to prompt formatting invalidate.
    brief: jobBrief(job) };
}

function jobFingerprint(job) { return fingerprint(jobInput(job)); }

// Bound concurrent WebCrypto work; reuse unchanged objects only within a page.
// No fingerprint is accepted before its description has arrived.
const prepared = new WeakMap();
function jobFields(job) {
  return [job.title, job.department, job.description_text, job.employer_name,
    job.location, job.remote, job.salary_min, job.salary_max];
}
async function prepareJobs(jobs) {
  for (let i = 0; i < jobs.length; i += 200) {
    await Promise.all(jobs.slice(i, i + 200).map(async (job) => {
      if (job._descPending) { job._judgmentHash = null; return; }
      const fields = jobFields(job);
      const prior = prepared.get(job);
      if (prior && fields.every((value, i) => value === prior.fields[i])) { job._judgmentHash = prior.hash; return; }
      job._judgmentHash = null;
      const hash = await fingerprint(jobInput(job));
      const current = jobFields(job);
      if (job._descPending || !fields.every((value, i) => value === current[i])) return;
      prepared.set(job, { fields, hash });
      job._judgmentHash = hash;
    }));
  }
}

return {
  JUDGMENT_VERSION, DEFAULT_MODEL, stableStringify, fingerprint,
  profileFingerprint, jobFingerprint, prepareJobs, contractFingerprint,
