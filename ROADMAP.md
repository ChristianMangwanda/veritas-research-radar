...
| 3.2 | Interfolio driver + promote wiring — **investigated 2026-08-03, harder than scoped.** `apply.interfolio.com/<id>` is an AngularJS 1.7 SPA backed by several private REST hosts (`api.interfolio.com`, `secure.interfolio.com`, `logic.interfolio.com`…) — needs either a Playwright scout path or real API reverse-engineering, not a plain-fetch driver. 19 hits confirmed so far in the ongoing discovery re-crawl (JHU, Yale, Case Western, Rochester, Georgetown…); full count pending. Deferred to its own session. | `refresh.js` or scout | 1d+ |
