...
| 0.7 | **Added 2026-08-04:** 0.6's detector flagged 21 employers on its first night live — found a real metric conflation bug (comparing prefiltered_count against the FINAL post-scoring `fetched_jobs` instead of the prefilter's own pass-through), fixed it, dropped the count to 3 (all confirmed benign). Also: Greenhouse/SmartRecruiters probes had no identity check at all (same class of gap as Workday's, fixed earlier the same session) — fixed both, documented Lever/Ashby as an accepted gap (no identity signal exists in their APIs). Also: registry gained `secondary_ats_feeds` support — an employer can now have two ATS feeds (e.g. a Workday staff board + a separate Interfolio faculty board); 6 wired after live verification (Rochester, Georgetown, Baylor, San Diego State, Wake Forest, Hillsdale) | `refresh.js`, `promote-employers.js` | ✅ + tests |
...
| 3.7 | Discovery crawl moved off-laptop and onto CI — **new `radar-discover.yml` workflow, 2026-08-04.** Was a local `scout_discover.py` process (slow: ~7h to clear the ~4,471-site backlog); now runs in GitHub Actions (free, public repo), monthly (after the enrichment run that feeds it fresh candidates) plus on-demand via `workflow_dispatch`. GitHub-hosted runners hard-cap a job at 6h, so a single run may not clear a large backlog — the crawl already saves progress per-site and skips recrawls, so it resumes cleanly across runs with no manual bookkeeping. | `.github/workflows/radar-discover.yml` | ✅ automated |
